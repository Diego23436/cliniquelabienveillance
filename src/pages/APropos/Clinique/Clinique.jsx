        ...
        <div className="locations-grid">
          {c.locations.map((loc) => (
            <div className="location-card" key={loc.name}>
              <h3>{loc.name}</h3>
              <p className="location-line"><strong>{loc.address}</strong></p>
      ...
      <section className="team-teaser">
        <div className="container team-teaser-inner">
          ...
          <div className="team-teaser-actions">
            <Link to="/a-propos/equipe" className="btn btn-primary">{c.teamCta}</Link>
            <Link to="/services" className="btn btn-outline-blue">{c.servicesCta}</Link>
          </div>
        </div>
      </section>
    </PageShell>
